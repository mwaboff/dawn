...
export class SubclassPathService {
  ...
  getSubclassPaths(): Observable<CardData[]> {
    const params = new HttpParams().set('expand', 'associatedDomains,spellcastingTrait');

    return this.http
      .get<SubclassPathApiResponse[]>(this.baseUrl, { params, withCredentials: true })
      .pipe(map(responses => responses.map(mapSubclassPathToCardData)));
  }
}
